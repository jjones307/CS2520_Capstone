...
def generate(lang_choice):
    ...
    while (len(generated_name) < name_length):
        if next_letter == 'v':
            choice = \
            random.choices(['v', 'vg'], weights=[rel_frequencies['v'] - rel_frequencies['vg'], rel_frequencies['vg']])[
                0]
            if choice == 'v':
                ...
                group_to_add = \
                random.choices(list(vowel_group_frequency.keys()), weights=list(vowel_group_frequency.values()))[0]
                generated_name += group_to_add
            next_letter = 'c'
        elif next_letter == 'c':
            choice = \
            random.choices(['c', 'cg'], weights=[rel_frequencies['c'] - rel_frequencies['cg'], rel_frequencies['cg']])[
                0]
            if choice == 'c':
                letter_to_add = \
                random.choices(list(consonant_frequency.keys()), weights=list(consonant_frequency.values()))[0]
                generated_name += letter_to_add
            else:
                group_to_add = random.choices(list(consonant_group_frequency.keys()),
                                              weights=list(consonant_group_frequency.values()))[0]
                generated_name += group_to_add
            next_letter = 'v'
